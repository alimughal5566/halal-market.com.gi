...
class CheckoutController extends Controller
{
    ...
    public function checkout()
    {
        ...
        if (Auth::guard('web')->check()) {
            ...
            // dd($shippingPrice);                
            return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => $shippingPrice, 'digital' => $dp, 'curr' => $curr, 'shipping_data' => $shipping_data, 'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);
        } else {
                ...
                foreach ($products as $prod) {
                    if ($prod['item']['type'] != 'Physical') {
                        if (!Auth::guard('web')->check()) {
                            ...
                            return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => $shippingPrice, 'checked' => $ck, 'digital' => $dp, 'curr' => $curr, 'shipping_data' => $shipping_data, 'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);
                        }
                    }
                }
                $shipping = 0;
    ...
    public function cashondelivery(Request $request)
    {
        ...
        foreach ($cart->items as $prod) {
            if ($prod['item']['user_id'] != 0) {
                $price = $prod['price'] - ($gs->fixed_commission + (($prod['price'] / 100) * $gs->percentage_commission));
        ...
        if (!empty($notf)) {
            ...
            foreach ($users as $user) {
                ...
                if ($vendor->is_vendor == 2) {
                    $to = $vendor->email;
                    $subject = "New Order Recieved!!";
                    $msg = "Hello Vendor!<br>Your store has received a new order.<br>Order Number is " . $order->order_number . ".Please login to your panel to check. <br>Thank you.";
