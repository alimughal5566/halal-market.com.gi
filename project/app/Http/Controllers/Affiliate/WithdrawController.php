...
class WithdrawController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('affiliate.withdraw.index',compact('withdraws','sign'));
    }

    public function create()
    {
        $sign = Currency::where('is_default','=',1)->first();
        return view('affiliate.withdraw.create' ,compact('sign'));
    }
...
//            $amount = round(($amount / $curr->value),2);
            if ($from->current_balance >= $amount+$charge){
//                $fee = (($withdrawcharge->withdraw_charge / 100) * $amount) + $charge;
                $fee = $charge;
                $finalamount = ($amount+$fee) - $fee;
                ...
                $newwithdraw['fee'] = $fee;
//                $newwithdraw['type'] = 'affiliate';
                $newwithdraw->save();
//                return response()->json('Withdraw Request Sent Successfully.');
                return redirect()->back()->with('success', ([ 0 => 'Insufficient BalanceWithdraw Request Sent Successfully.' ]) );

            }else{
//                return response()->json(array('errors' => [ 0 => 'Insufficient Balance.' ]));
                return redirect()->back()->with('errors', ([ 0 => 'Insufficient Balance.' ]) );
            }
        }
//        return response()->json(array('errors' => [ 0 => 'Please enter a valid amount.' ]));
        return redirect()->back()->with('errors', ([ 0 => 'Please enter a valid amount.' ]) );
